...
browseURL("https://docs.google.com/presentation/d/e/2PACX-1vRTSSO_8JuLTK_1OyM9eDrogA-K2fhXQwlKxh1PpRvNavkurCCcKBNftv9MpKGYM6EDXtNnqZvPDdKy/pub?start=false&loop=false&delayms=30w00&slide=id.g2c8ba90947_0_0")
...
word_pairs <- unnested.df %>%
    pairwise_count(words, title, sort = TRUE)
...
```{r tdm}
library(tidytext)
dtm <- cast_dtm(doc_term_count, title, words, n)
...
adjacent_list = lapply(1:16, function(i){as.data.frame(embed(terms.df[,i], 2)[, 2:1])})
...
m <- cast_sparse(doc_term_count, title, words, n)
# dim(m)
# class(m)
...
## The 2nd solution
* convert an lda result to json for ldavis
```
...
* topic modeling比較適合類似新聞的文章，長度夠長，一篇文章內有多個主題，比較不適合類似社群資料等短文。而CTM（Correlated Topic Modeling）則是設計用來處理短文的主題分析。
...
=======
# Correlated Topic Modeling
>>>>>>> cc5078cb44f160d5de0cd078587d1a5d24e3fd98
...
>>>>>>> 9dd5b6071a86a4cda63c7606e540b5f38c6eb9fa
