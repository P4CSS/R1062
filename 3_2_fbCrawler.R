...
token <- "188730144854871|1lL4a4CTRymAHvoKxnJDQqvqVdc"
...
# 4. (option) do.call() to combine data chunks ----------------------------
...
res <- fromJSON(content(GET(fburl), "text"))
all.list <- list()
all.list[[1]] <- res$posts$data
next_url <- res$posts$paging$"next"

i <- 2 # Index of data chunk. The next chunk goes from i == 2.

while(!(is.null(next_url))){
	next_res <- fromJSON(content(GET(next_url), "text"))
	all.list[[i]] <- next_res$data
	next_url <- next_res$paging$"next"
	message(i)
	i <- i + 1
}
...
comment.df$from.name <- comment.df$from$name # create new var
comment.df$from.id <- comment.df$from$id # create new var
comment.df$from <- NULL # delete the hierarchical variable

next_res$data$from.name <- next_res$data$from$name # create new var
next_res$data$from.id <- next_res$data$from$id # create new var
next_res$data$from <- NULL # delete the hierarchical variable
...
for(pid in all.df$id){
	...
	message(sprintf("[%d]%s-%s", pi, pid, format(Sys.time(), "%Y%m%d-%H%M%S")))
	pi <- pi + 1
}
